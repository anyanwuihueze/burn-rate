...
// FIX 1: Never inject inside a multi-line import block
...
async function collectFeatureTags(byFile, cwd) {
  ...
  console.log('\n' + bold(magenta('🏷  Feature Tagging (v2.0.5)')) + '\n');
...
const SDK_CONTENT = `// BurnRate SDK v2.0.5 — auto-installed by burnrate-init
...
async function main() {
  ...
  console.log('\n' + bold(cyan('⚡ BurnRate Init')) + gray(' v2.0.5'));
  ...
  const source  = await downloadSdk(sdkPath);
  sdkSpin.succeed(green('SDK ready') + gray(` → ${rel(cwd, sdkPath)} (${source})`));
